...
//  printf("Here 5 virtual_address_cache_entry: %d\n", virtual_address_cache_entry->index);

//  printf("addr: %p\n", &(eviction->congruent_address_cache[virtual_address_cache_entry->index]));
  // Run eviction
  if (eviction->congruent_address_cache[virtual_address_cache_entry->index].used == true)
...
//        printf("Here %d %d %d addr %p..\n", i, j, k,
//            congruent_address_cache_entry->congruent_virtual_addresses[i+k]);
    ...
    eviction, size_t index, uintptr_t physical_address)
{
  ...
  printf("Found %d of %d, index: %lu\n", found, ADDRESS_COUNT, index);
...
//  assert(found == ADDRESS_COUNT);
  ...
  if (found == ADDRESS_COUNT)
    ...
    eviction->congruent_address_cache[index].used = false;
